%% 这个文件根据CMFD历史时期（1985-2014）计算得到的基尼系数和年降水量
%% 在气候区上求区域平均
%% 在四个时间尺度上画散点拟合图
...
%         % 做拟合线
        ...
        eval(['X = [ones(length(AP_',climate_zone_name{5-j},'),1),AP_',climate_zone_name{5-j},'];'])
...
%         eval(['plot(AP_',climate_zone_name{5-j},',GI_',Temporal_scale_name{i},'_',climate_zone_name{5-j},',''k.'')'])
        eval(['scatter(AP_',climate_zone_name{5-j},',GI_',Temporal_scale_name{i},'_',climate_zone_name{5-j},',15,''k'',''filled'')'])
        hold on
        if b(2) > 0
        ...
        % 加title，标子图标题+拟合线
        if stats(3) < 0.01
            text('string',[' K** = ',num2str(roundn(b(2),-5),'%4.5f'),'mm^{-1}'],'Units','normalized','position',[0.12,1.11],'fontsize',15,'FontName','times new Roman')
        elseif stats(3) < 0.05
            text('string',[' K* = ',num2str(roundn(b(2),-5),'%4.5f'),'mm^{-1}'],'Units','normalized','position',[0.12,1.11],'fontsize',15,'FontName','times new Roman')
        else
            text('string',[' K = ',num2str(roundn(b(2),-5),'%4.5f'),'mm^{-1}'],'Units','normalized','position',[0.12,1.11],'fontsize',15,'FontName','times new Roman')
...
% cd('F:\File_of_MATLAB\6-硕士毕业论文\4-pic\005-02-scatter-plot-of-AR-GI-CMFD-1985-2014\')
% exportgraphics(gcf,'历史时期基尼系数和年降水量的散点拟合关系图.jpg')