...
% 计算相关系数和P值
PCCs_MAT_GI_climatic_zone_Areamean = nan(4,4); % 横轴表示时间尺度的变化，纵轴表示气候区的变化
P_MAT_GI_climatic_zone_Areamean = nan(4,4); % 横轴表示时间尺度的变化，纵轴表示气候区的变化

for i = 1 : length(Temporal_scale_name)
    for j = 1 : length(climate_zone_name)
        
        eval(['[PCCs_1,P_1] = corrcoef(MAT_',climate_zone_name{j},', GI_',Temporal_scale_name{i},'_',climate_zone_name{j},');'])
        
        PCCs_MAT_GI_climatic_zone_Areamean(i,j) = PCCs_1(2);
        P_MAT_GI_climatic_zone_Areamean(i,j) = P_1(2);
...
Table = num2cell(PCCs_MAT_GI_climatic_zone_Areamean);
for i = 1 : size(Table,1)
    for j = 1 : size(Table,2)
        aa = Table{i,j};
        PP = P_MAT_GI_climatic_zone_Areamean(i,j);
        % 判断相关系数是否显著
        if PP < 0.01
...
cd('F:\File_of_MATLAB\6-硕士毕业论文\4-pic\010-02-Table-PCCs_bewteen_climate_zone_Areamean_MAT_and_GI\')
filename = 'Table-4.2.1.xls';
