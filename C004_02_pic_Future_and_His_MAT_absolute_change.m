%% 这个文件根据未来两个时期和历史时期中年均温气候态的绝对变化率进行画图
%% 画绝对变化率
...
for j = 1 : length(SSP_type)
    for m = 1 : length(Time_period)
        save_peth_1 = 'J:\6-硕士毕业论文\1-Data\CMIP6-Tas-Pr-2015-2100\9-2-Absolute-change-of-Future-and-His-MAT\';
        cd(save_peth_1)
        filename_1 = ['Absolute_change_',SSP_type{j},'_',Time_period{m},'_025_scale_climatic_MAT.mat'];
        load(filename_1)
        
        eval(['Absolute_change_ssp370_2031_2060_all(:,:,j) = Absolute_change_',SSP_type{j},'_',Time_period{m},';'])
...
b1 = min(min(min(Absolute_change_ssp370_2031_2060_all)));
a1 = max(max(max(Absolute_change_ssp370_2031_2060_all)));


mask_CN_climatic_MAT_Absolute_change(Lon_025,Lat_025,Absolute_change_ssp126_2031_2060,Absolute_change_ssp245_2031_2060,...
    Absolute_change_ssp370_2031_2060,Absolute_change_ssp585_2031_2060,shp0,shp,shp1,shp2,-2,8,shp_line_HR,shp_line_TR,shp_line_AR,shp_line_TP)

cd('F:\File_of_MATLAB\6-硕士毕业论文\4-pic\002-07-CMIP6-Future-and-His-MAT-Absolute-change\')
exportgraphics(gcf,'未来时期和历史时期年均温气候态的绝对变化率.jpg')
% close all